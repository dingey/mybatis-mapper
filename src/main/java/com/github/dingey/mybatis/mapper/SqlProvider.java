...
public class SqlProvider {
    ...
    public String delete(Object bean) {
        return getCachedSql(bean, "delete", t -> {
            StringBuilder sql = new StringBuilder();
            sql.append("delete from ").append(JPA.table(bean)).append(" where ");
            List<String> ids = new ArrayList<>();
            try {
                for (Field field : getCachedModelFields(bean.getClass())) {
                    if (field.isAnnotationPresent(Id.class)) {
    ...
    public String get(Object bean) {
        return getCachedSql(bean, "get", t -> {
            StringBuilder sql = new StringBuilder();
            sql.append("select * from ").append(JPA.table(bean)).append(" where ");
            try {
    ...
    public String list(Object bean) {
        StringBuilder sql = new StringBuilder();
        sql.append("select * from ").append(JPA.table(bean)).append(" where 1=1 ");
        String orderby = null;
        try {
            for (Field f : getCachedModelFields(bean.getClass())) {
    ...
    public String listAll(Class<?> bean) {
        return getCachedSql(bean, "listAll", (Func<Class<?>>) t -> "select * from " + JPA.table(bean));
    }
    ...
    public String countAll(Class<?> bean) {
        return getCachedSql(bean, "countAll", (Func<Class<?>>) t -> "select count(0) from " + JPA.table(bean));
    }
    ...
    public String listByIds(Class<?> entity, Iterable<Serializable> ids) {
        ...
        s.append("select * from ").append(JPA.table(entity)).append(" where ");
